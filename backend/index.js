...
const connectToMongo = async () => {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      // Check IOT connection (imported from cornAnalysis.service.js)
      await iotConnection.asPromise();
      console.log('Connected to IOT MongoDB successfully via corn analysis service');
...
app.get('/health', async (req, res) => {
  ...
  const isIotConnected = iotConnection.readyState === 1;
  ...
  res.status(200).json({
    ...
    mongodb_iot: isIotConnected ? 'connected' : 'disconnected',
...
// Set up automatic Python analytics every 5 minutes
cron.schedule('*/5 * * * *', async () => {
  console.log('Running scheduled Python analytics...');
  try {
    await AnalyticsService.runPythonAnalytics();
    const prescription = await AnalyticsService.getLatestPrescription();
    if (prescription) {
      console.log('Scheduled analytics completed successfully:', {
        timestamp: prescription.timestamp,
        severity: prescription.severity_level,
        recommendation: prescription.overall_recommendation
      });
    } else {
      console.log('No new prescriptions generated');
    }
  } catch (error) {
    console.error('Error in scheduled Python analytics:', error);
  }
});
    ...
    const gracefulShutdown = async (signal) => {
      ...
      server.close(async () => {
        try {
          ...
          if (iotConnection) await iotConnection.close();
