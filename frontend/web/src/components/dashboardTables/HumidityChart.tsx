      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-1 bg-white border border-[#356B2C] rounded shadow-md text-xs hidden group-hover:block z-10">
            <button onClick={() => handleExport("pdf", chartRef.current, chartData, xKey, "Humidity")} className="block px-4 py-2 hover:bg-[#E6F0D3] w-full text-left">PDF</button>
            <button onClick={() => handleExport("csv", chartRef.current, chartData, xKey, "Humidity")} className="block px-4 py-2 hover:bg-[#E6F0D3] w-full text-left">CSV</button>
            <button onClick={() => handleExport("svg", chartRef.current, chartData, xKey, "Humidity")} className="block px-4 py-2 hover:bg-[#E6F0D3] w-full text-left">SVG</button>
          </div>
        </div>
      </div>
