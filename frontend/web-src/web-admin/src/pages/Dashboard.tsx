...
// Mock data for other dashboard elements (keeping non-user data)
const mockData = {
  admin: {
    name: "John Smith"
  },
...
const AdminDashboard = () => {
    ...
    useEffect(() => {
      const fetchFarms = async () => {
        try {
          ...
          console.log("✅ Total Farms from API:", count); // <--- debug log
          setTotalFarms(count);
        } catch (err) {
  ...
  // Calculate active users (users who have logged in recently - you can adjust this logic)
  const getActiveUsers = () => {
    // For now, let's assume 60-70% of total users are active
    // You can replace this with real logic based on last login timestamps
    const activePercentage = 0.65; // 65% active rate
    return Math.floor(users.length * activePercentage);
  };
  const checkForNewActivity = async () => {
    if (!lastActivityCheck) return;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 mb-8">
          {[
            ...
            //   bg: "bg-orange-50", 
            //   activePercent: Math.round((data.stats.activeDevices / data.stats.totalDevices) * 100) 
            // }
          ].map((stat, i) => (
            <div key={i} className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="font-bold text-[#356B2C] mb-1 ml-1 text-5xl" >{stat.value}
              </div>
            </div>
          ))}
        </div>
