...
const LightIntensityDashboard = () => {
  ...
  const fetchData = async (period: string, baseDate: Date = new Date(), silent: boolean = false) => {
    ...
    try {
      ...
      if (!result.success || !result.data) {
        throw new Error(result.message || 'Failed to fetch light intensity data');
      }

      const processedData: DataItem[] = result.data.map((item: any) => ({
        ...item,
        value: item.lightIntensity ?? null,
      ...
      if (result.data.length > 0) {
        ...
        switch (period) {
          ...
          case 'weekly':
            ...
            break;
          case 'monthly':
            if (firstItem.monthStart && lastItem.monthEnd) displayRange = formatDateRange(new Date(firstItem.monthStart), new Date(firstItem.monthEnd));
            break;
        }
      }
      if (!displayRange) {
  ...
  const renderChart = () => {
    ...
    const thresholdLines = [
      <ReferenceLine key="min" y={LIGHT_INTENSITY_THRESHOLDS.min} stroke="green" strokeDasharray="3 3" />,
      <ReferenceLine key="max" y={LIGHT_INTENSITY_THRESHOLDS.max} stroke="orange" strokeDasharray="3 3" />,
      <ReferenceLine key="critical" y={LIGHT_INTENSITY_THRESHOLDS.critical} stroke="red" strokeDasharray="3 3" />,
    ];
