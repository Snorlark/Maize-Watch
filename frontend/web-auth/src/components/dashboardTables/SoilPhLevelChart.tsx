...
const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const getDataForTimeFrame = async (): Promise<{ data: DataItem[]; key: string }> => {
    ...
    try {
      const { chartData: newData, xKey: newKey } = await fetchAndFormatData(timeFrame, 'soilPh');
...
const SoilPhLevelChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const { chartData: newData, xKey: newXKey } = await fetchAndFormatData(overview, 'soilPh');
